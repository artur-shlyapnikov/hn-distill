...
export async function fetchItem(_services: Services, id: number): Promise<HnItemRaw | null> {
...
const CacheSchema = z.record(
  ...
  z.object({
    seenKids: z.array(z.number()).optional(),
    seenTopLevel: z.array(z.number()).optional(),
    seenByDepth: z.record(z.string().regex(/^\d*$/), z.array(z.number())).optional(),
